...
func TestPluginSecretServiceEnvKeyTakesPriorityOverKeyFile(t *testing.T) {
	...
	// env key로 시크릿 암호화
	if _, setErr := envSvc.SetSecret(kitsuPluginID, manifest, "access_token", "secret-value"); setErr != nil {
		t.Fatalf("SetSecret() error = %v", setErr)
	}
...
func TestPluginSecretServiceReusesKeyFileOnRestart(t *testing.T) {
	...
	if _, setErr := svc1.SetSecret(kitsuPluginID, manifest, "access_token", "secret-value"); setErr != nil {
		t.Fatalf("SetSecret() error = %v", setErr)
	}
