...
func (s *PluginSecretService) AutoGenerated() bool {
...
func NewPluginSecretService(cfg *config.Config, repo repository.PluginSecretRepository) (*PluginSecretService, error) {
	...
	if !keyFromEnv {
		keyFilePath := filepath.Join(cfg.DataDir, ".plugin_secret_key")
		...
		if seed == "" {
			...
			seed = base64.StdEncoding.EncodeToString(keyBytes)
			if err := os.MkdirAll(cfg.DataDir, 0700); err != nil {
				return nil, fmt.Errorf("create data dir for plugin secret key: %w", err)
			}
			if err := os.WriteFile(keyFilePath, []byte(seed), 0600); err != nil {
				return nil, fmt.Errorf("save plugin secret key: %w", err)
			}
			log.Printf("WARN: PLUGIN_SECRET_KEY not set — auto-generated key saved to %s (back this up)", keyFilePath)
		}
	}

	sum := sha256.Sum256([]byte(seed))
	return &PluginSecretService{cfg: cfg, repo: repo, key: sum[:], autoGenerated: !keyFromEnv}, nil
}
